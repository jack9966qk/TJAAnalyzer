...
test.describe("Chart List Playdata Display Modes", () => {
  ...
  const modes = [
    ...
    },
    {
      ...
      leading: "scoreRank",
      trailing: "none",
      name: "Crown + ScoreRank",
      snapshotName: "crownWithScoreRank",
    },
    {
      strip: "dnCategory",
      leading: "none",
      trailing: "none",
      name: "DN Style",
      snapshotName: "dnStyle",
    },
    {
      strip: "dnCategory",
      leading: "none",
      trailing: "counts",
      name: "DN Style + Counts",
      snapshotName: "dnStyleWithCounts",
    },
  ];

  for (const { strip, leading, trailing, name, snapshotName } of modes) {
    test(`renders ${name} correctly with variety`, async ({ page }) => {
      ...
      await page.evaluate(
        ({ strip, leading, trailing }) => {
          ...
          localStorage.setItem(
            ...
            JSON.stringify({
              chartListStripMode: strip,
              chartListLeadingMode: leading,
              chartListTrailingMode: trailing,
            }),
          );
      ...
      await expect(component).toHaveScreenshot(`chart-list-${snapshotName}.png`);
    });
  }
});
