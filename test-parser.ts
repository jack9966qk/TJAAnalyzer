...
`;
    ...
    if (branchChart.branches.normal.bars.length !== 6) throw new Error(`Normal branch should have 6 bars, got ${branchChart.branches.normal.bars.length}`);
    ...
    if (!branchChart.branches.normal.barParams[4].isBranched) throw new Error("Normal Bar 4 should be flagged as branched");
    if (branchChart.branches.normal.barParams[3].isBranched) throw new Error("Normal Bar 3 (Common) should NOT be flagged as branched");
...
`;
    ...
    if (!seqChart || !seqChart.branches || !seqChart.branches.normal) throw new Error("Sequential Branch Chart not parsed correctly");
