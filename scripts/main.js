...
function updateModeStatus(mode) {
    if (chartModeStatus) {
        if (mode === 'view')
            chartModeStatus.innerText = i18n.t('mode.view');
        else if (mode === 'judgements')
            chartModeStatus.innerText = i18n.t('mode.judgements');
        else if (mode === 'selection')
            chartModeStatus.innerText = i18n.t('mode.selection');
        else if (mode === 'annotation')
            chartModeStatus.innerText = i18n.t('mode.annotation');
    }
}
function switchDisplayOptionTab(mode) {
    doTabs.forEach(t => {
        if (t.getAttribute('data-do-tab') === mode)
        ...
        tjaFilePicker.addEventListener('change', async (event) => {
            ...
            if (files && files.length > 0) {
                ...
                try {
                    ...
                    const msg = i18n.t('status.parseError', { error: e.message || String(e) });
                    alert(msg);
                    if (statusDisplay)
                        statusDisplay.innerText = msg;
                }
