...
export class ExportButton extends HTMLElement {
    ...
    generateContent() {
        ...
        return generateTJAFromSelection(appState.currentChart, appState.viewOptions.selection, courseBranchSelect.difficulty, this.loopCount, this.chartName, this.gapCount);
    }
    async handleClick() {
        if (this.disabled || this.status !== "idle")
            return;
        if (!appState.currentChart || !appState.viewOptions.selection)
            return;
        try {
    ...
    async transitionToResult(action) {
        this.isFading = true;
        this.render();
    ...
    async exportToDirectoryNeutralino(content, path, runSave) {
    ...
    render() {
        ...
        const vdom = (_jsxs("div", { children: [_jsx("link", { rel: "stylesheet", href: "style.css" }), _jsx("button", { type: "button", className: className, onclick: this.handleClick.bind(this), disabled: this.disabled || this.status !== "idle", style: `width: 100%; transition: opacity 0.15s ease-out, background-color 0.15s; opacity: ${this.isFading ? "0" : "1"}; cursor: ${this.status === "idle" ? "pointer" : "default"}`, children: content })] }));
        if (this.shadowRoot) {
